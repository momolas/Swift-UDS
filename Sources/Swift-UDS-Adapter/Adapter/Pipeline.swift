...
extension UDS {
    ...
    public final actor Pipeline {
        ...
        public static let LoggingDirectory = FileManager.CC_pathInCachesDirectory(suffix: "dev.cornucopia.Swift-UDS")

        public var logFilePath: String? = nil
        nonisolated public let adapter: UDS.Adapter
        private var logFile: FileHandle? = nil
        private lazy var logQ: OperationQueue = {
            let q = OperationQueue()
            q.name = "dev.cornucopia.Swift-UDS.Pipeline.Logging"
        ...
        public func startLogging(header: String = "") {
            ...
            let dir = Self.LoggingDirectory
            do {
                try FileManager.default.createDirectory(atPath: dir, withIntermediateDirectories: true, attributes: nil)
                let logFilePath = "\(dir)/\(UUID()).csv"
                FileManager.default.createFile(atPath: logFilePath, contents: nil, attributes: nil)
                self.logFilePath = logFilePath
                self.logFile = FileHandle(forWritingAtPath: logFilePath)
                guard self.logFile != nil else { throw NSError(domain: "dev.cornucopia.Swift-UDS", code: 7, userInfo: nil) }
        ...
        public func stopLogging() {
            ...
            self.logQ.waitUntilAllOperationsAreFinished()
            do {
                try logFile.close()
            } catch {
        ...
        deinit {
            ...
            self.stopLogging()
            logger.debug("Pipeline shutdown completed.")
        }
    }
}

extension UDS.Pipeline: CustomStringConvertible {

    nonisolated public var description: String { "<UDS.Pipeline -> \(self.adapter)>" }
}
