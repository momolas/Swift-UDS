...
public extension UDS.GenericSerialAdapter {
    ...
    enum StringCommand: Equatable {
        ...
        case tryProtocol(p: UDS.BusProtocol)

        // extended (STN-compatible)
        case stnCanSegmentationAddSeparationTime(ms: Float)
        ...
        case stnTransmitAnnounce(header: UDS.Header? = nil, count: Int, expectedResponseCount: Int? = nil)
        ...
        // UDS
        case data(bytes: [UInt8], expectedResponseCount: Int? = nil)
        case test(bytes: [UInt8])
    }
}
