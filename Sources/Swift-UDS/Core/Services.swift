...
public extension UDS {
    ...
    /// UDS Service Requests including their parameters.
    enum Service {
        ...
        // KWP 2000 & GMLAN specific
        case kwpClearDiagnosticInformation(groupOfDTC: GroupOfDTC = 0xFFFF)
        case kwpReadDataByLocalIdentifier(id: DataIdentifier8)
        case kwpReadDTCByStatus(type: KWPReadDTCByStatusType = .requestAllDTCAndStatus, groupOfDTC: GroupOfDTC = 0xFFFF)
        case kwpReadEcuIdentification(id: DataIdentifier8)
        case kwpTesterPresent

        // Custom
        case custom(serviceId: UInt8, subfunction: UInt8, optionRecord: DataRecord)
        ...
        public var payload: [UInt8] {
            ...
            switch self {
                ...
                case .freezeFrameData(frame: let frame, pid: let pid):
                    return [UDS.ServiceId.powertrainFreezeFrameData, pid, frame]
                ...
                case .kwpClearDiagnosticInformation(let group):
                    let (_, _, lsbhi, lsblo) = group.CC_UInt8tuple
                    return [UDS.ServiceId.clearDiagnosticInformation, lsbhi, lsblo]
                    
                case .kwpReadDataByLocalIdentifier(let id):
                    return [UDS.ServiceId.kwpReadDataByLocalIdentifier, id]

                case .kwpReadDTCByStatus(let type, let group):
                    let (_, _, lsbhi, lsblo) = group.CC_UInt8tuple
                    return [UDS.ServiceId.kwpReadDTCsByStatus, type.rawValue, lsbhi, lsblo]

                case .kwpReadEcuIdentification(let id):
                    return [UDS.ServiceId.kwpReadECUIdentification, id]

                case .kwpTesterPresent:
                    return [UDS.ServiceId.kwpTesterPresent]

                    //
                    // Custom
                    //
                case .custom(let serviceId, let subfunction, let optionRecord):
                    return [serviceId, subfunction] + optionRecord
            }
        }
    }
}
